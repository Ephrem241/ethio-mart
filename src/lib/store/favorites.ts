...
import { addRemoteFavorites, removeRemoteFavorite } from "@/lib/services/favorites-remote"
  ...
  persist(
    (set, get) => ({
      ...
      toggle: (id) => {
        ...
        // Signed in: also write through to the `favorites` table.
        const user = useAuthStore.getState().user
        if (user && get().ownerId === user.id) {
          if (wasFavorited) void removeRemoteFavorite(user.id, id)
          else void addRemoteFavorites(user.id, [id])
        }
      },
